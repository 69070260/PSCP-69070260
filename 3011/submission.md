...
```text
Input:
ผมเขียนรับค่า input ให้มันเป็น str อยู่แล้วเพราะมันเป็นตัวอักษรไม่ใช่ตัวเลข ในโจทย์เค้าให้เขียนรับ input มา 2 บรรทัด ผมก็เขียนตามนั้น
Output:
output มีหลายค่า ผมจึงเขียนค่าให้มันครอบคลุมทุก input ที่เข้ามาจนครบทุกแบบ
Constraints:
ค่าที่รับมามันเยอะมากๆ ผมจึงแยกออกมาทุกๆกรณี และห้ามลืมว่าผสมสีเดิมแล้วต้องได้สีเดิมด้วย ถ้าผสมสีอื่นที่ไม่ใช่แม่สีผสมกันเองจะต้องเป็น Error
```
...
```text
Step 1 : ผมรับค่า str มา ค่าที่รับมาจะมีแค่ Red Blue Yellow เท่านั้น
Step 2 : แบ่งออกมาหลายๆกรณี ทุกๆสีที่ผสมกัน
Step 3 : ห้ามลืมว่าสีเดิมผสมกันจะได้สีเดิม
Step 4 : เขียนโค้ดลงไป ลองรันหลายๆ case ว่ามีแบบไหนอีกมั้ยที่ยังผิด
...
```text
คล้ายๆแผนแรก แต่มีข้อผิดพลาดตอนแรกอยู่ว่าทำไมบางโค้ดมันยังผิด แต่ก็เจอตรงที่ว่าทำไมถึงผิด ผมผิดตรง ต้องเขียน elif แต่ดันไปเขียน if เฉยเลย
```
...
```text
เพราะสีเดิมเวลาผสมต้องได้สีเดิม
```
...
```text
สีอื่นที่ไม่ใช่แม่สีผสมต้องได้ Error
```
...
```text
ผสมสี Yellow และ Red ผสมกันต้องได้ Orange
```
